...
async function loadUniverseEntry(
  asset: number,
): Promise<{ universe: HlMeta["universe"]; localIdx: number } | null> {
...
export async function placeOrders(
  ...
  if (
    (grouping === "normalTpsl" || grouping === "positionTpsl") &&
    placedOids.length > 1
  ) {
    ...
    if (childOids.length > 0) {
      ...
      for (const childOid of childOids) {
        ...
        if (sibs.length > 0)
          pipeline.sadd(KEYS.ORDER_BRACKET(childOid), ...sibs);
      }
    }
    await pipeline.exec();
  }
...
// ─── twapOrder ────────────────────────────────────────────────────────────────
// Generates a twapId, stores metadata in Redis, schedules sub-orders
// every 30 seconds over m minutes. Each slice = totalSize / numSlices
// placed as an IOC order at the current mid price.
...
export async function createTwapOrder(
  ...
  // Generate twapId via Redis auto-increment (same pattern as OID/TID)
  ...
  const sliceSz = parseFloat(sliceSzNum.toFixed(szDecimals)).toString();
  ...
  // Schedule sub-orders in the background
  let slicesFired = 0;
  const interval = setInterval(async () => {
    try {
      // Check if cancelled before each slice
      ...
      // Place slice as IOC at current mid price
      ...
      logger.info(
        { twapId, slicesFired, numSlices, status },
        "TWAP slice executed",
      );
